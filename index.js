...
/*Task 1: counterMaker()
  ...
  1. What is the difference between counter1 and counter2?
	 Counter1 has the placeholder "let" at the function scoped. Counter1 also has a closure which accesses the 
	 placeholder in the outer function scoped. While counter2 only has a global scoped placeholder.
	 
  2. Which of the two uses a closure? How can you tell?
	 Counter1 uses the closure "return function counter(){}"
	 
  3. In what scenario would the counter1 code be preferable? In what scenario would 
     counter2 be better?  
	 Counter2 is a regular function to avoid repetition. Counter1 is almost the same
	 but it looks that slows down the app more than a regular function. Because of that 
	 functions with closures are limited.
*/
...
function inning(){
  return Math.round(Math.random() * Math.round(3));
} 
...
function scoreboard(inningScoreCB, inningCB, numInnings) {
  ...
  for (let i = 0; i < numInnings; i++){
	  var currentInning = inningScoreCB(inningCB);
